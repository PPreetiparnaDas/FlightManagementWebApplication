				...
				<div class="modal-header">
				  ...
				  <div class="modal-body">
				  	<form id="updateAdmin">
				        ...
				        <input type="submit" value="Update Admin"  class="btn btn-info form-control  form-control-lg mb-3" >	
					</form>	
				  </div>	
					</div>
				</div>
                   ...
                   if(response.hasOwnProperty(key)){
						...
						if(status=="active"){
							 s+="<tr>";
		                	   ...
		                	   s+="<a  class='text-primary AdminEdit'  id='"+response[key].uId+"' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Edit'><i class='bi bi-pencil-fill'></i></a>";
		                	   s+="<a  class='text-danger AdminDelete' id='"+response[key].uId+"' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete'><i class='bi bi-trash-fill'></i></a>";
		                	   s+="</div>";
		                	   s+="</td>";
		                	   s+="</tr>";
						}else{
...
<script>
	$(document).on('click','.AdminDelete',function(){
		...
		if(confirm("Are you sure to deactivate this ?")){
		    ...
		    url: "../AdminServlet",
		    type : "Post",
			data:{"uId":uId,"event":"deactiveAdmin"},
			success:function(response){
...
<script>
	$(document).on('click','.AdminEdit',function(){
		...
		$.ajax({ 
			type : "Post",
			url:"../AdminServlet",
			data:{"uId":uId,"event":"getAdminById"},
			dataType:"json",
			success:function(response){
				console.log(response);
